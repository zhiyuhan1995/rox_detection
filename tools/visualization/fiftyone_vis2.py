...
def main(args):
    try:
        if os.path.exists("saved_predictions_view2") and os.path.exists("saved_filtered_view2"):
            ...
            predictions_view = fo.Dataset.from_dir(
                dataset_dir="saved_predictions_view2",
                dataset_type=fo.types.FiftyOneDataset
            ).view()
            filtered_view = fo.Dataset.from_dir(
                dataset_dir="saved_filtered_view2",
                dataset_type=fo.types.FiftyOneDataset
            ).view()
        else:
            dataset = foz.load_zoo_dataset(
            ...
            predictions_view.export(
                export_dir="saved_predictions_view2",
                dataset_type=fo.types.FiftyOneDataset
            )
            ...
            anno_image_paths = predictions_view.draw_labels(
                "saved_predictions_view2",
                label_fields=label_field,     # only predictions
            )
